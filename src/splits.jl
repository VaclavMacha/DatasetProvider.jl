...
struct TrainTest <: Split
    ...
    function TrainTest(;
        ...
        return new(at, standardize, seed)
    end
end
...
function (split::TrainTest)(
    ...
    if !hastest(N)
        train, test = datasplit(train, obsdim, split.at)
    else
...
function (split::TrainValidTest)(
    ...
    if !hasvalid(N) && !hastest(N)
        train, valid, test = datasplit(train, obsdim, at)
    elseif !hastest(N)
        train, test = datasplit(train, obsdim, sum(at))
        valid = load_raw(N, :valid)
    elseif !hasvalid(N)
        train, valid = datasplit(train, obsdim, sum(at))
        test = load_raw(N, :test)
    else
        valid = load_raw(N, :valid)
        test = load_raw(N, :test)
